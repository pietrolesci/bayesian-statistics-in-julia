...
begin
    ...
    analytical_posterior_predictive =
        BetaBinomial(births, fem_births + 1, births - fem_births + 1)
...
begin
    ...
    plot!(
        analytical_prior_predictive_dist,
        lw = 2,
        label = "Analytical prior predictive (BetaBinomial)",
    )
    hline!(
        [analytical_prior_predictive],
        lw = 2,
        colour = :red,
        label = "Analytical prior predictive",
    )
    ...
    plot!(
        analytical_posterior_predictive,
        seriestype = :line,
        label = "Analytical posterior predictive",
        lw = 2,
    )
...
begin
    ...
    annotate!(
        ...
        0.018,
        Plots.text(
            "p-value=$(round(mean(posterior_predictive[:y] .> fem_births), digits=3))",
            10,
        ),
    )
