...
begin
    ...
    plot(
        θs, [pdf(Binomial(10, θ), 5) for θ in θs], 
        legend=:topright, 
        seriestype=:line,
        ylabel="Likelihood",
        xlabel=L"\theta",
...
begin
    θs = range(0., stop=1., length=1000)
    plot(
        θs, [-loglikelihood(Binomial(10, θ), 5) for θ in θs], 
        legend=:top, 
        seriestype=:line,
...
begin
    ns = 0:50
    plot(
        ns, [pdf(Binomial(n, .5), 5) for n in ns], 
        marker=:dot, 
        seriestype=:line,
        ylabel="Likelihood",
        xlabel=L"n",
...
begin
    ns = 0:50
    plot(
        ns, [-loglikelihood(Binomial(n, .5), 5) for n in ns], 
        legend=:topleft,
        marker=:dot, 
        seriestype=:line,
        ylabel="Negative log-likelihood",
...
begin
    ...
    heatmap(A, c=:inferno, xlabel=L"\theta", ylabel=L"n", xformatter=x->"$(x/1000)")
