...
begin
    ...
    plot!(
        posterior_normal_approx_sample,
        label = "Normal approx using samples",
        seriestype = :scatter,
        marker = :star,
    )
end
...
begin
    ...
    # to compute 95% credible intervals
    a_normal_approx_sample_logit =
        quantile(posterior_normal_approx_sample_logit, α / 2) |> logistic |> round3
    b_normal_approx_sample_logit =
        quantile(posterior_normal_approx_sample_logit, 1 - α / 2) |> logistic |> round3
...
begin
    ...
    θs = range(0.001, 1.0, step = 0.001) |> collect
    prior = fill(1.0, length(θs))
